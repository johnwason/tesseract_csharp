...
%define SWIG_STD_MAP_INTERNAL(K, T, C, ALLOC)

%typemap(csinterfaces) std::map< K, T, C, ALLOC > "global::System.IDisposable \n    , global::System.Collections.Generic.IDictionary<$typemap(cstype, K), $typemap(cstype, T)>\n";
%proxycode %{
  ...
  public:
    ...
    %extend {
      const mapped_type& getitem(const key_type& key) throw (std::out_of_range) {
        std::map< K, T, C, ALLOC >::iterator iter = $self->find(key);
        if (iter != $self->end())
          return iter->second;
        else
      ...
      %apply void *VOID_INT_PTR { std::map< K, T, C, ALLOC >::iterator *create_iterator_begin }
      %apply void *VOID_INT_PTR { std::map< K, T, C, ALLOC >::iterator *swigiterator }

      std::map< K, T, C, ALLOC >::iterator *create_iterator_begin() {
        return new std::map< K, T, C, ALLOC >::iterator($self->begin());
      }

      const key_type& get_next_key(std::map< K, T, C, ALLOC >::iterator *swigiterator) {
        std::map< K, T, C, ALLOC >::iterator iter = *swigiterator;
        (*swigiterator)++;
        return (*iter).first;
      }

      void destroy_iterator(std::map< K, T, C, ALLOC >::iterator *swigiterator) {
        delete swigiterator;
      }
    }
...
namespace std {
  template<class K, class T, class C = std::less<K>, class Allocator =  std::allocator<std::pair<const K, T> > > class map {
    SWIG_STD_MAP_INTERNAL(K, T, C, Allocator)
  };
}
