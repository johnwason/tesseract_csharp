...
%define SWIG_STD_VECTOR_MINIMUM_INTERNAL(CSINTERFACE, CONST_REFERENCE, CTYPE, ALLOC)
%typemap(csinterfaces) std::vector< CTYPE, ALLOC > "global::System.IDisposable, global::System.Collections.IEnumerable, global::System.Collections.Generic.CSINTERFACE<$typemap(cstype, CTYPE)>\n";
%proxycode %{
  ...
  public:
    ...
    %extend {
      vector(int capacity) throw (std::out_of_range) {
        std::vector< CTYPE, ALLOC >* pv = 0;
        if (capacity >= 0) {
          pv = new std::vector< CTYPE, ALLOC >();
          pv->reserve(capacity);
       } else {
      ...
      void AddRange(const std::vector< CTYPE, ALLOC >& values) {
        $self->insert($self->end(), values.begin(), values.end());
      }
      // Takes a deep copy of the elements unlike ArrayList.GetRange
      std::vector< CTYPE, ALLOC > *GetRange(int index, int count) throw (std::out_of_range, std::invalid_argument) {
        if (index < 0)
          throw std::out_of_range("index");
        if (count < 0)
        ...
        return new std::vector< CTYPE, ALLOC >($self->begin()+index, $self->begin()+index+count);
      }
      ...
      void InsertRange(int index, const std::vector< CTYPE, ALLOC >& values) throw (std::out_of_range) {
        if (index>=0 && index<(int)$self->size()+1)
      ...
      static std::vector< CTYPE, ALLOC > *Repeat(CTYPE const& value, int count) throw (std::out_of_range) {
        if (count < 0)
          throw std::out_of_range("count");
        return new std::vector< CTYPE, ALLOC >(count, value);
      }
      void Reverse() {
      ...
      void SetRange(int index, const std::vector< CTYPE, ALLOC >& values) throw (std::out_of_range) {
        if (index < 0)
          throw std::out_of_range("index");
...
%define SWIG_STD_VECTOR_EXTRA_OP_EQUALS_EQUALS(CTYPE, ALLOC)
    %extend {
      ...
      int IndexOf(CTYPE const& value) {
        int index = -1;
        std::vector< CTYPE, ALLOC >::iterator it = std::find($self->begin(), $self->end(), value);
        if (it != $self->end())
      ...
      int LastIndexOf(CTYPE const& value) {
        int index = -1;
        std::vector< CTYPE, ALLOC >::reverse_iterator rit = std::find($self->rbegin(), $self->rend(), value);
        if (rit != $self->rend())
...
%define SWIG_STD_VECTOR_ENHANCED(CTYPE, ALLOC)
namespace std {
  template<> class vector< CTYPE, ALLOC > {
    SWIG_STD_VECTOR_MINIMUM_INTERNAL(IList, const value_type&, %arg(CTYPE), %arg(ALLOC))
    SWIG_STD_VECTOR_EXTRA_OP_EQUALS_EQUALS(%arg(CTYPE), %arg(ALLOC))
  };
}
%enddef

// Legacy macros
%define SWIG_STD_VECTOR_SPECIALIZE(CSTYPE, CTYPE, ALLOC)
...
SWIG_STD_VECTOR_ENHANCED(%arg(CTYPE), %arg(ALLOC))
%enddef

%define SWIG_STD_VECTOR_SPECIALIZE_MINIMUM(CSTYPE, CTYPE, ALLOC)
...
namespace std {
  ...
  template<class T, class Allocator =  std::allocator<T> > class vector {
    SWIG_STD_VECTOR_MINIMUM_INTERNAL(IEnumerable, const value_type&, %arg(T), %arg(Allocator))
  };
  // specialization for pointers
  template<class T, class Allocator =  std::allocator<T> > class vector<T *, Allocator> {
    SWIG_STD_VECTOR_MINIMUM_INTERNAL(IList, const value_type&, %arg(T *), %arg(Allocator))
    SWIG_STD_VECTOR_EXTRA_OP_EQUALS_EQUALS(%arg(T *),%arg(Alloc))
  };
